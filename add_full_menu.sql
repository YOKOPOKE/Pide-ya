-- =============================================
-- ADD FULL MENU DATA (IDEMPOTENT)
-- =============================================

-- Cleanup Categories (Optional: If needing fresh start for these specific items)
DELETE FROM products WHERE category IN ('Pokes de la Casa', 'Share & Smile', 'Drinks', 'Postres');

-- TYPE MAPPING:
-- 'poke' -> Default Builder (3 steps)
-- 'burger' -> Burger Builder (2 steps)
-- 'other' -> Simple add to cart (0 steps)

-- 1. POKES DE LA CASA (Type 'poke' because they technically ARE pokes, but maybe pre-configured? 
-- User said "no se condicionan". So let's make them type 'other' (Simple) but category 'Pokes de la Casa'.
-- Or we could make them 'fixed_poke'. For now, 'other' is safest for "Add to Cart" behavior.)
