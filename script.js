...
function startBuilder() {
    if (!currentProduct.type || !currentProduct.size) {
        showToast('Selecciona un producto y tamaño', '⚠️');
        return;
    }
    ...
    showToast(`${productName} ${config.name} - $${config.basePrice}`, '✅');
}
...
async function fetchMenuData() {
    ...
    if (sizes) {
        ...
        if (!SIZE_CONFIG['small']) {
            ...
            SIZE_CONFIG['small'] = { name: 'Pequeño', basePrice: 120, maxProteins: 1, mixins: 3, crunches: 2, sauces: 1, priceMixin: 10, priceSauce: 10, priceCrunch: 15, priceProtein: 40 };
        }

    } else {
        // Safe defaults
        SIZE_CONFIG = {
            small: { name: 'Pequeño', basePrice: 120, maxProteins: 1, mixins: 3, crunches: 2, sauces: 1 },
...
function renderCheckoutUpsell() {
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
        <h3 class="font-bold text-yoko-dark mb-4 text-sm uppercase tracking-wider flex items-center gap-2">
            <i class="fa-solid fa-plus-circle text-yoko-accent"></i> ¿Algo más?
        </h3>
    ...
    if (type === 'base') {
        ...
        if (index > -1) {
            ...
            if (type === 'proteins') {
                ...
                if (price > 0) {
                    showToast(`Proteína extra: +$${price}`, '💰');
                } else {
                    showToast(`Agregada: ${value} `, '🐟');
                }
                playSound('pop');
            } else {
                arr.push(value);
                const iconMap = { mixins: '🥗', sauces: '🍯', toppings: '🥜' };
                // Check Limits
                let limit = 99;
                let cost = 0;
                ...
                if (arr.length > limit && cost > 0) {
                    showToast(`Costo extra: +$${cost}`, '💰');
                } else {
                    showToast(`Agregado: ${value} `, iconMap[type] || '✅');
                }
                playSound('pop');
            }
        }
...
function validateStep(shake = false) {
    ...
    if (currentStep === 0 && !currentOrder.base) {
        ...
        isValid = false;
        msg = 'Selecciona al menos una proteína';
...
function renderSummary() {
        ...
        html += `
            <div class="py-2 border-b border-white/10">
                <div class="flex items-center gap-2 mb-2">
                    <span class="text-xs font-bold text-gray-400 uppercase tracking-widest pl-1">Proteínas</span>
    ...
    renderSection('Mixins (Vegetales)', 'mixins', config.mixins || 99, config.priceMixin || 0, '🥗', 'bg-green-500/20 text-green-200');
    renderSection('Crunchs', 'crunches', config.crunches || 99, config.priceCrunch || 0, '🥜', 'bg-amber-600/20 text-amber-200');
    renderSection('Salsas', 'sauces', config.sauces || 99, config.priceSauce || 0, '🍯', 'bg-yellow-500/20 text-yellow-200');
    ...
    if (currentOrder.extras && currentOrder.extras.length > 0) {
        ...
        currentOrder.extras.forEach(item => {
            total += item.price;
            let icon = '✨';
            if (item.category === 'drink') icon = '🥤';
            if (item.category === 'side') icon = '🥢';
            if (item.category === 'dessert') icon = '🍰';
    ...
    if (missing.length > 0) {
        ...
        if (statusDiv) {
            ...
            statusDiv.innerHTML = `<i class="fa-solid fa-check-circle"></i> <span>¡Listo!</span>`;
        }
        if (orderBtn) {
...
function sendWhatsAppOrder() {
    ...
    setTimeout(() => {
        ...
        let text = `🧾 *NUEVO PEDIDO - YOKO POKE HOUSE* 🧾\n\n`;
        ...
        if (currentOrder.proteins.length) {
            ...
            currentOrder.proteins.forEach(p => {
                ...
                text += `  • ${p.name}${costInfo}\n`;
            });
            text += `\n`;
        }

        if (currentOrder.mixins.length) {
            text += `🥗 *MIXINS (${currentOrder.mixins.length})*\n`;
        ...
        if (currentOrder.sauces.length) {
            text += `🍯 *SALSAS (${currentOrder.sauces.length})*\n`;
...
function renderCheckoutSummary() {
    ...
    if (currentOrder.proteins.length) {
        html += `<div class="py-1.5 border-b border-gray-100">
            <span class="text-xs text-gray-400 uppercase tracking-wide block mb-1">Proteínas</span>
            <div class="space-y-1">`;
        currentOrder.proteins.forEach(p => {
            const icon = getIcon('proteins', p.name);
            html += `
...
function submitOrder() {
    ...
    if (!name || !phone) {
        showToast('Por favor, ingresa tu Nombre y Teléfono', '📋');
        return;
    }
    ...
    const productIcon = currentProduct.type === 'bowl' ? '🥣' : '🍔';
    text += `${productIcon} *PRODUCTO:* ${productName} ${sizeName}\n`;
    text += `💵 *PRECIO BASE:* $${currentProduct.basePrice}\n\n`;

    // Customer Info
    text += `👤 *CLIENTE:* ${name}\n`;
    text += `📱 *TEL:* ${phone}\n`;
    text += `🚚 *ENTREGA:* ${deliveryMethod === 'delivery' ? 'A DOMICILIO 🛵' : 'RECOGER EN SUCURSAL 🏪'}\n`;

    if (deliveryMethod === 'delivery') {
        text += `📍 *DIRECCIÓN:* ${address}\n`;
        if (refs) text += `ℹ️ *REF:* ${refs}\n`;
    }
    ...
    if (currentOrder.base) text += `🍚 *BASE:* ${currentOrder.base}\n`;
    ...
    if (currentOrder.mixins.length) text += `🥗 *MIXINS:* ${currentOrder.mixins.join(', ')}\n`;
    if (currentOrder.sauces.length) text += `🍯 *SALSAS:* ${currentOrder.sauces.join(', ')}\n`;
    if (currentOrder.toppings.length) text += `🥜 *TOPPINGS:* ${currentOrder.toppings.join(', ')}\n`;
...
function showToast(message, icon = '✅') {
    ...
    toast.innerHTML = `
        ...
        <div class="flex flex-col">
            ...
            <span class="text-[10px] text-gray-400 font-medium uppercase tracking-wider mt-0.5">Notificación</span>
        </div>
